...
module.exports = function(app) {
  ...
  app.get('/api/goals', function(req, res) {
    Goal.find({userID: req.body.userID}, function(err, docs) {
      if (err) {
        return res.status(400).send('Error retrieving goals: ' + err);
      }
      var goals = [];
      var allGoals = docs.forEach(function(obj) {
        console.log(obj);
        goals.push(obj);
      });
      Promise.resolve(allGoals)
      .then(res.status(200).send(JSON.stringify(goals)))
